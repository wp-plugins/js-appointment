...
class Calendar{
	...
	function __construct()
	{
        ...
        $this->arrViewTypes["search"]  = "search";
	...
	*/
	private function DrawMonth()
	{
		...
		echo "<tr>";
			echo "<th colspan='7'>";
            echo "<div class=\"table_navbar ts-1\">".$this->crLt;
			echo "<div class=\"tr_navbar_top ts-1-1\">&nbsp;&nbsp; ".SearchFront()."</div>".$this->crLt;
			echo "<div class=\"tr_navbar ts-1-2\">".$this->crLt;
            ...
            echo "</div>".$this->crLt;
			echo "<div class=\"tr_navbar_right ts-1-3\">
			...
			echo "<div class=\"gen-1\"></div>".$this->crLt;
		    echo "</div>".$this->crLt;
			echo "</td>".$this->crLt;
		echo "</tr>".$this->crLt;
...
$avv = NOTVVS;
    ...
    print "<tr>";
    echo "<td>".$dates." - ".$times[$key] = date('H:i:s', $time)."</td>";
    echo "<td>".$row['max']." ".$avv."</td>";
    print "</tr>";
}
print "</table>";
...
$search = "<div class='newadd'><form name='form' action='".$url=get_bloginfo('wpurl')."/index.php?page_id=".$post_id."&action=view&view_type=search' method='post' >";
$search .= '<input name="day" type="text" id="date" value="Search for a date" onfocus="this.value=(this.value==\'Search for a date\') ? \'\' : this.value;" onblur="this.value=(this.value==\'\') ? \'Search for a date\' : this.value;"/>&nbsp;&nbsp;&nbsp;&nbsp;';
$search .= 'And for &nbsp;&nbsp;<select name="cat" size="1" class="category">';
$category = mysql_query("SELECT * FROM wp_resservation_cat");
while($row1 = mysql_fetch_array($category))
        {
            /*** create the options ***/
            $search .= '<option value="'.$row1['id'].'"';
            $search .= '>'. $row1['name'] . '</option>'."\n";
        }


$jumpcat .= '</select>';
$search .= '<input type="submit" class="bottonego" name="Search" value="go" style="margin-left:15px; color:#ffffff;"/>';
//$search .= '<button class="update" type="submit">Update cart</button>';
$search .= '</form></div>';
