...
$items = explode(',',$cart);
$s = (count($items) > 1) ? 's':'';
return '<p>You have <a href="'.$url.'/index.php?page_id='.$settings["allbook_pageid"].'&pager=cart">'.count($items).' item'.$s.' in the shopping cart</a></p>';
}
}
...
$output[] = '<form action="'.$url.'/index.php?page_id='.$settings["allbook_pageid"].'&pager=cart&action=update" method="post" id="cart">';
...
if ($max > $qty) {
$output[] = '<tr>';
$output[] = '<td class="tdcart"><a href="'.$url.'/index.php?page_id='.$settings["allbook_pageid"].'&pager=cart&action=delete&id='.$id.'" class="r">'.REMOVE.'</a></td>';
$output[] = '<td class="tdcart">&nbsp;&nbsp;<span class="tablespan">'.HOURS.'</span> &nbsp;'.$time_start.' &nbsp;&nbsp;<span class="tablespan"></span> '.$date.'</td>';
$output[] = '<td class="tdcart"><span class="tablespan">'.PRICE.'</span> &nbsp;'.$settings["allbook_valute_simbolo"].''.$price.'</td>';
...
$output[] = '<td class="tdcart"><span class="tablespan">'.PRICE.' '.$settings["allbook_valute_simbolo"].'</span>'.$price.'</td>';
...
$output[] = '</form>';
$output[] = '<form action="'.$url.'/index.php?page_id='.$settings["allbook_pageid"].'&pager=checkout&action=checkout" method="post" id="cart">';
foreach ($contents as $id=>$qty) {
$sql = 'SELECT * FROM wp_resservation_disp WHERE id = '.$id;
$result = mysql_query($sql);
//$row = $result->getArray();
...
$output[] = '<td><input name="price['.$id.']" type="hidden" value="'.$price.'" /> <input name="category['.$id.']" type="hidden" value="'.$category.'" /></td>';
$total1 += $price * $qty;
} else {
$output[] = '<td><input name="id['.$id.']" type="hidden" value="'.$id.'" /></td>';
...
echo '<div class="erroroptions">Error select one options</div><br />';
}
break;
case 'delete':
if ($cart) {
