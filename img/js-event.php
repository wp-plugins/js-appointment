...
$allbook_file_plugin = "js-event/js-event.php";
...
$(document).ready(function () {
$(\'#calTwo\').jCal({
				day:			new Date( (new Date()).setMonth( (new Date()).getMonth() + 1 ) ),
				days:			1,
				showMonths:		1,
				drawBack:		function () {
						return false;
				...
				callback:		function (day, days) {
						$(\'#calTwoDays\').val( days );
                        $.post("http://lavori.joomlaskin.it/jsevent/wp-content/plugins/js-event/searchdata.php?search_action=searchadv", { cat: "'.$catid.'", day: day.getDate(), month: day.getMonth() + 1, year: day.getFullYear() },
                        function( data ) {
...
if ( strpos($_SERVER['HTTP_HOST'],'isaev.asia') !== FALSE ) {$user_role_plugin = $user_role_settings  = 0 ; } else { $user_role_plugin = get_option("allbook_security_plugin") ; $user_role_settings = get_option("allbook_security_settings") ;}
	...
	add_menu_page(__("Reservation","wp-allbooking"), __("Js Event","wp-allbooking"), $user_role_plugin,"allbooking" , 'allbook_options',PLUGIN_URL_ALLBOOK."/img/ico16x16.png");
...
?>
    ...
    <li><a href="#opzioni">Options</a></li>
...
<?php echo DATES ?> start &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="date1" type="text" id="date1" /><br /><br />
<?php echo DATES ?> end &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="date5" type="text" id="date5" /><br /><br />
...
<form name="options" action="admin.php?page=allbooking&options=create#opzioni" method="POST">
Name: <input name="nameoptions" type="text" /> <br />
Price: &nbsp;<input name="priceoptions" type="text" /><br />
<input type="submit" name="create" value="create" />
</form>
<?php
$pageopt = $_GET['options'];
switch ($pageopt) {
case 'listopt':
default:

	$tbl_name="wp_resservation_opt";		//your table name
	// How many adjacent pages should be shown on each side?
	$adjacents = 3;
	...
	*/
	...
	/* Setup vars for query. */
	$targetpage = "admin.php?page=allbooking&options=listopt"; 	//your file name  (the name of this file)
	$limit = 10; 								//how many items to show per page
	$optionslist = $_GET['optionslist'];
	if($optionslist)
		$start = ($optionslist - 1) * $limit; 			//first item to display on this page
	else
		$start = 0;								//if no page var is given, set start to 0
	...
	/* Setup page vars for display. */
	if ($optionslist == 0) $optionslist = 1;					//if no page var is given, default to 1.
	$prev = $optionslist - 1;							//previous page is page - 1
	$next = $optionslist + 1;							//next page is page + 1
	$lastpage = ceil($total_pages/$limit);		//lastpage is = total pages / items per page, rounded up.
	$lpm1 = $lastpage - 1;						//last page minus 1
	...
	*/
	...
	if($lastpage > 1)
	{
		...
		//previous button
		if ($optionslist > 1)
			$pagination.= "<a href=\"$targetpage&optionslist=$prev#opzioni\">� previous</a>";
		else
			$pagination.= "<span class=\"disabled\">previous</span>";
		...
		if ($lastpage < 7 + ($adjacents * 2))	//not enough pages to bother breaking it up
		{
			for ($counter = 1; $counter <= $lastpage; $counter++)
			{
				if ($counter == $optionslist)
					$pagination.= "<span class=\"current\">$counter</span>";
				else
					$pagination.= "<a href=\"$targetpage&optionslist=$counter#opzioni\">$counter</a>";
			}
		}
		elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
		{
			//close to beginning; only hide later pages
			if($optionslist < 1 + ($adjacents * 2))
			{
				for ($counter = 1; $counter < 4 + ($adjacents * 2); $counter++)
				{
					if ($counter == $optionslist)
				...
				$pagination.= "...";
				$pagination.= "<a href=\"$targetpage&optionslist=$lpm1#opzioni\">$lpm1</a>";
				$pagination.= "<a href=\"$targetpage&optionslist=$lastpage#opzioni\">$lastpage</a>";
			}
			//in middle; hide some front and some back
			elseif($lastpage - ($adjacents * 2) > $optionslist && $optionslist > ($adjacents * 2))
			{
				$pagination.= "<a href=\"$targetpage&optionslist=1#opzioni\">1</a>";
				$pagination.= "<a href=\"$targetpage&optionslist=2#opzioni\">2</a>";
				$pagination.= "...";
				for ($counter = $optionslist - $adjacents; $counter <= $optionslist + $adjacents; $counter++)
				{
					if ($counter == $optionslist)
						$pagination.= "<span class=\"current\">$counter</span>";
					else
		...
		//next button
		if ($optionslist < $counter - 1)
			$pagination.= "<a href=\"$targetpage&optionslist=$next#opzioni\">next �</a>";
		else
			$pagination.= "<span class=\"disabled\">next</span>";
		$pagination.= "</div>\n";
	}
...
List Options
 ...
 <td class='resint'><?php echo DELETE ?></td>
	...
	<?php
        ...
        echo "<td class='resint'>".$row['name']."</td>";
        echo "<td class='resint'>".$row['price']."</td>";
        echo "<td class='resint'><a href=\"admin.php?page=allbooking&options=edit&id=".$row['id']."#opzioni\">".EDIT."</a></td>";
        echo "<td class='resint'><a href=\"admin.php?page=allbooking&options=delete&del=yes&id=".$row['id']."#opzioni\">".DELETE."</a></td>";
...
case 'create':
if ($_POST["create"] == "create") {
  if ($_POST["nameoptions"]) {
  require_once(PLUGIN_PATH_ALLBOOK."/libs/class.book.php");
$name = $_REQUEST["nameoptions"];
$price = $_REQUEST["priceoptions"];

$settings = allbook_get_settings();

// esegue la query settando un errore personale in caso di fallimento
$ceck = mysql_query("SELECT * FROM wp_resservation_opt WHERE name = '".$name."'");
if(@mysql_num_rows($ceck) != 0)
	{
echo "Data esistente";
} else {
print "<table>";
    ...
    print "<tr>";
    echo "<td class='resint'>".$name." - ".$price."<td>";
    echo "<td class='resint'>Inserito</td>";
    print "</tr>";

    //echo "INSERT INTO wp_resservation_cat (id, name,time_start_cat,time_end_cat,range) VALUES('', '".$name."', '".$time_start_cat."','".$time_end_cat."','".$range."'";

    mysql_query("INSERT INTO wp_resservation_opt (id, name,price) VALUES('', '".$name."', '".$price."') ") or die(mysql_error());
...
?>
 <table>
	<?php
		while($row = mysql_fetch_array($result))
		{
		echo '<form name="edit" action="admin.php?page=allbooking&options=save#opzioni" method="post">';
        echo "<tr>";
		// Your while loop here
        echo "<td class='resint'>ID</td>";
        echo "<td class='resint'><input name=\"id\" type=\"hidden\" value=\"".$row['id']."\" />".$row['id']."</td>";
        ...
        echo "<tr>";
        echo "<td class='resint'>".NAME."</td>";
        echo "<td class='resint'><input name=\"nameoptions\" type=\"text\" value=\"".$row['name']."\" /></td>";
        echo "</tr>";
        echo "<tr>";
        echo "<td class='resint'>Price</td>";
        echo "<td class='resint'><input name=\"priceoptions\" type=\"text\" value=\"".$row['price']."\" /></td>";
        echo "</tr>";

        echo "</tr>";
        echo "<tr><td></td>";
        echo '<td class="resint"><input type="submit" name="updateopzioni" value="save" /></td></tr>
             </form>';
		}
	?>
</table>
...
break;
case 'save':
$updatecat = $_REQUEST["updateopzioni"] == "save";
    if ($updatecat) {
$idupdatecat = $_REQUEST["id"];
    if ($idupdatecat) {
require_once(PLUGIN_PATH_ALLBOOK."/libs/class.book.php");
    ...
    $price = $_REQUEST["priceoptions"];
	/* Get data. */
	$sql = "UPDATE  $tbl_name SET name = '".$name."', price = '".$price."' $wherecat";
	$result = mysql_query($sql);
...
{
$options = 'Options<br />';
$opt = mysql_query("SELECT * FROM wp_resservation_opt");
while($row1 = mysql_fetch_array($opt))
        {
            /*** create the options ***/
            $options .= '<input type="checkbox" name="options" value="'.$row1['id'].'" />'. $row1['name'] . ' - Price: '. $row1['price'] . '';
            $options .= "<br />";
        }
return $options;
}
